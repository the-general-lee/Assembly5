...
void validate_inputs (unsigned int cs,unsigned int ls,char * cache_mode, unsigned int crc,unsigned int cwc ,unsigned int mrc ,unsigned int mwc)
{
        ...
        if ( cwc == 0 )
        {
                printf ("Cache Write Cycles need to be a value larger than zero\n");
                exit(1);
        }
        if ( mrc == 0 )
        {
                printf ("Memory Read Cycles need to be a value larger than zero\n");
                exit(1);
        }
        if ( mwc == 0 )
        {
                printf ("Memory Write Cycles need to be a value larger than zero\n");
                exit(1);
        }
